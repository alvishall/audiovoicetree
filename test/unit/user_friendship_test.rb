...
context "#mutual_friendship" do
  setup do
      ...
      @friendship1 = users(:jason).user_friendships.where(friend_id: users(:mike).id).first
      @friendship2 = users(:mike).user_friendships.where(friend_id: users(:jason).id).first
    end
